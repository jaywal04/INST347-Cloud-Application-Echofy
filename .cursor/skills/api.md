...
| GET | `/api/spotify/top-tracks` | Optional user/OAuth | Query **`chart`:** `auto` (default), `spotify_personal` (only your Spotify tops — tries short-, medium-, and long-term; omits **local-file** tops; **429** ⇒ `spotify_rate_limited`), `lastfm_tracks`, `lastfm_artists`, `lastfm_geo`. Auto prefers personalized Spotify when connected, else Last.fm overall (matched to Spotify). Other Last.fm modes use `chart.getTopTracks`, `chart.getTopArtists` + `artist.getTopTracks`, or `geo.getTopTracks`. Requires `LAST_FM_API_KEY` + Spotify app credentials for non-personal paths; geo country from `LAST_FM_GEO_COUNTRY` or `SPOTIFY_MARKET` mapping. |
